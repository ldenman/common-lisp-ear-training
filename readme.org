...
I have these functions in my ~/.emacs file to quickly reload the project:
   ...
   #+begin_src elisp
     ...
     (defun ld-music-load ()
       ...
       (slime)
       (sit-for 2)
       (slime-eval '(ql:quickload "ld-music"))
       (slime-eval-async '(ld-music:my-midi-setup)))

     ;https://github.com/slime/slime/issues/17
     ...
     (advice-add 'slime-eval-async :around #'slime-repl-skip-eval-when-reading)
   #+end_src
